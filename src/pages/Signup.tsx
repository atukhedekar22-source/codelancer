...
const Signup = () => {
  ...
  useEffect(() => {
    if (user && userProfile && !authLoading) {
      if (userProfile.role === 'developer') navigate('/developer');
      else if (userProfile.role === 'freelancer') navigate('/freelancer');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Signup request timed out. Please check your internet connection and try again.')), 15000);
      });
      ...
      // Navigation will be handled by the useEffect above once the user profile is loaded
    } catch (err: any) {
      ...
      console.log('[Signup] Cleanup loading state');
      setLoading(false);
    }
  };
          ...
          <div className="bg-card rounded-3xl p-8 border border-border shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <button
                ...
                className={`p-4 rounded-xl border-2 transition-all ${role === 'developer'
